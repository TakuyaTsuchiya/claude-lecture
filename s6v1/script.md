...
2つ目は**Vite + .env**。Viteは現代の標準的なビルドツールで、`.env`というファイルに**環境変数**、つまりアプリの外から渡す設定値（特に鍵などの秘密情報）を書いて、ブラウザに安全に渡してくれます。`.env`は`.gitignore`でGitHubには送らないようにします。鍵は家の鍵と同じで、SNSに載せてはいけません。
