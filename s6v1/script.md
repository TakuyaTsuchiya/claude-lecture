...
2つ目は**Vite + .env**。Viteは現代の標準的なビルドツールで、`.env`ファイルに鍵を書いておくと、ブラウザから安全に使えるようにしてくれます。この鍵のことを**環境変数**と呼びます。`.env`は`.gitignore`でGitHubには送らないようにします。鍵は家の鍵と同じで、SNSに載せてはいけません。
