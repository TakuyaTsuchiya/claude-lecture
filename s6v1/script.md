...
2つ目は**Vite + .env**。Viteは現代の標準的なビルドツールで、`.env`ファイルに鍵を書いておくと、ブラウザから安全に使えるようにしてくれます。この鍵のことを**環境変数**と呼びます。`.env`は`.gitignore`でGitHubには送らないようにします。
...
続いて、アプリデータの保存場所を、localStorageからSupabaseに変えます。プロンプトは長いので、概要欄に貼っておきます。
...
Claudeが作った`.env`ファイルを開いて、先ほどコピーしたProject URLとPublishable keyを貼ります。
...
ここで`VITE_SUPABASE_URL`と`VITE_SUPABASE_PUBLISHABLE_KEY`の2つを登録します。登録を忘れるとビルドが失敗するので、push前に必ず済ませておきます。
...
スライドに出しているように、`createClient`で鍵を渡して接続を作り、`select`で読み取り、`insert`で作成、`update`で更新、`delete`で削除します。

ご自身のアプリで同じことをしたいときは、Claude Codeに自然言語で頼むだけです。今回のように「Vite導入・Supabase接続・テーブル定義のSQL生成」といった技術的な実装も、まとめて引き受けてくれます。プロンプトは概要欄に貼っておきます。
...
アプリ名やテーブル名を自分のものに置き換えれば、データベース連携を実装できます。

SQLには**RLS（Row Level Security）の有効化**とポリシーも含めました。RLSがないとPublishable keyだけで誰でもDBにアクセスできてしまうので、本番運用では必須です。今回は最小ポリシーなので、実サービスでは認証済みユーザーのみに制限するなど、Supabase公式ドキュメントを参考に厳密に設定してください。
