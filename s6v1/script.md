...
> RLSを無効化するSQLと、初期データ3件のINSERTも含めてください。
...
なお、今回はRLS（Row Level Security）という機能を無効にしました。これは学習用の最小構成で、実サービスとして公開するときはRLSを有効にしてポリシーを書く必要があります。詳しい設定はSupabase公式ドキュメントを参照してください。
