...
今回もテーブル作成のSQLにRLS有効化とポリシー設定を含めました。Publishable keyは公開前提なので、それだけだとブラウザに埋め込まれた鍵で誰でもDBにアクセスできてしまいます。2テーブルあるので、それぞれにポリシーが必要です。今回は最小限のポリシーですが、本番運用では認証済みユーザーのみが自分の顧客データを操作できる、のような厳密な制約をかけます。詳しい書き方はSupabase公式ドキュメントを参照してください。
