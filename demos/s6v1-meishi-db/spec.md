...
- 接続情報は `.env` から `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` を読む
...
| `VITE_SUPABASE_ANON_KEY` | 匿名キー | 同上 |

`.env` は `.gitignore` により GitHub には送られない。Vercel側には別途登録が必要で、**初回デプロイは環境変数未登録で失敗する**のが仕様（教材上の自然な山場）。

---

## 7. 対象外の機能

- ユーザー認証・マルチユーザー（anon key で全員が同じデータを見る設計）
